...
public:
  ...
  /**
   * @brief reads the nth problem in the problem file and writes formatted
   ...
   */
  void readProblem(const std::string problems_filepath,
                   const int problem_number);

  /**
   * @brief reads the next problem in the problem file and writes formatted
   * contents to class members problem_matrix_ given an open filestream of test
   * problems.
   *
...
private:
  ...
  std::vector<int>
  getProblemRowAsIntVector(const std::string problem_row_string);

  std::vector<int> spliceVector(std::vector<int> to_splice,
