...
void Reader::readNextProblem(std::fstream &problems_filestream) {
  ...
  const int num_variables =
      static_cast<int>(atoi(num_variables_string.c_str()));

  // Get number of inequalities.
  std::getline(problems_filestream, num_inequality_rows_string);
  num_inequalities_ =
      static_cast<int>(atoi(num_inequality_rows_string.c_str()));
  ...
  for (size_t i = 0; i < num_inequalities_; ++i) {
    ...
    if (problem_matrix_row.size() != num_variables) {
      std::cout << "ERROR: length of matrix row is: " << problem_row.size()
                << " but length of " << num_variables << " was expected"
                << std::endl;
    }
...
void Reader::readProblem(const std::string problems_filepath,
  ...
  int num_equality_rows =
      static_cast<int>(atoi(num_equality_rows_string.c_str()));
...
std::vector<int>
Reader::convertStringToVector(const std::string vector_string) {
...
std::vector<int>
Reader::getProblemRowAsIntVector(const std::string problem_row_string) {
  std::string tempstring;
...
std::vector<int> Reader::spliceVector(std::vector<int> to_splice,
