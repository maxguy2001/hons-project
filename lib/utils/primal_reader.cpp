...
namespace utils{
  ...
  void PrimalReader::readProblem(const std::string path_to_primal_problem, const core::int_t problem_num){
  ...
  const core::int_t num_rows = static_cast<core::int_t>(atoi(num_rows_string.c_str()));
  const core::int_t num_vars = static_cast<core::int_t>(atoi(num_vars_string.c_str()));

  //define temp string and vector for holding 
  std::string temp_string;
  std::vector<core::int_t> temp_vec;
  ...
  std::vector<core::int_t> PrimalReader::convertStringToVector(const std::string vector_string){
