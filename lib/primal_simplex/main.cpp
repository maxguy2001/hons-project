...
namespace primal_simplex{
  ...
  std::vector<float> PrimalSimplex::extractColumnFromTable(const int column_index){
  ...
  int PrimalSimplex::getPivotColumnIndex(){
    ...
    auto min_value =  std::min_element(objective_values.begin(), objective_values.end());
    int min_index;
    for(size_t i = 0; i < objective_values.size(); ++i){
  ...
  int PrimalSimplex::getPivotRowIndex(const int pivot_column_index){
    ...
    const std::vector<float> pivot_column = extractColumnFromTable(pivot_column_index);
    const std::vector<float> bounds_column = extractColumnFromTable(table_.size()-1);
  ...
  void PrimalSimplex::switchBasis(const int pivot_row_index, const int pivot_column_index){
    //update basis
    basis_.at(pivot_row_index-1) = pivot_column_index;
  }
  

  void PrimalSimplex::constructNewTable(const int pivot_row_index, const int pivot_column_index){
    //define new table
    ...
    for(size_t i = 0; i < table_.size(); ++i){
      ...
      if(i == pivot_row_index){
        //divide all elements by pivot value and add ot new table
        for(size_t j = 0; j < pivot_row.size(); ++j){
          new_elem = pivot_row[j]/pivot_value;
      ...
      //case if pivot row element is already zero
        ...
        for(size_t j = 0; j < pivot_row.size(); ++j){
          new_elem = (old_row[j]*(product/pivot_column.at(i))) - (pivot_row[j]*(product/pivot_value));
          new_row.push_back(new_elem);
        }
      }
  ...
  void PrimalSimplex::printSolution(){
    ...
    std::vector<float> bounds = extractColumnFromTable(table_.size()-1);
    bounds.erase(bounds.begin());

    //print results to terminal
    for(size_t i = 0; i < bounds.size(); ++i){
      std::cout << "Variable " << basis_.at(i) << " = " << bounds.at(i) << std::endl;
    }
  ...
  void PrimalSimplex::solveProblem(){
    ...
    for(size_t i = 0; i < core::kMaxIterations; ++i){
      ...
      if(i == core::kMaxIterations-1){
        std::cout << "Max nuber of iterations reached. No solution found." << std::endl;
      }
    }
  }
