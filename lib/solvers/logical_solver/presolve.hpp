...
public:
  ...
  // CONSTRUCTOR
  Presolve(const std::vector<std::vector<int>> problem_matrix,
           const std::vector<int> lower_bounds,
           const std::vector<int> upper_bounds, const int inequalities_count,
           const int equalities_count, const bool solve_MIP);
...
private:
  ...
  int getDependancyIndexRowDoubletonMIP(const int row_index,
                                        const int col_index);

  bool checkConstraintByFasibleValue(const int row_index, const int col_index,
                                     const double feasible_value);

  bool checkDependancyMIP(const int feasible_value,
   ...
   * and an integer in the RHS. It checks if the RHS divided by the coefficient
   * is an integer that satisfies the implied bounds. If not, if it is an
   * inequality, tries rounding up to nearest integer and checks if the upper
   * bound is satisfied.
   *
   ...
   */
  int getVariableFeasibleValueMIP(const int row_index, const int col_index,
